...
`.leave`  |  Makes Shinobu leave the server. Either name or id required. **Bot Owner Only!**| `.leave 123123123331`
...
`.prune`, `.clr`  |  `.prune` removes all Shinobu's messages in the last 100 messages.`.prune X` removes last X messages from the channel (up to 100)`.prune @Someone` removes all Someone's messages in the last 100 messages.`.prune @Someone X` removes last X 'Someone's' messages in the channel. |  `.prune` or `.prune 5` or `.prune @Someone` or `.prune @Someone X`
...
`.stats`  |  Shows some basic stats for Shinobu. |  `.stats`
`.dysyd`  |  Shows some basic stats for Shinobu. |  `.dysyd`
...
`;permrole`, `;pr`  |  Sets a role which can change permissions. Or supply no parameters to find out the current one. Default one is 'Shinobu'. |  `;pr role`
...
`@BotName rip`  |  Shows a grave image of someone with a start year |  `@Shinobu rip @Someone 2000`
`@BotName die`  |  Works only for the owner. Shuts the bot down. |  `@Shinobu die`
`@BotName do you love me`  |  Replies with positive answer only to the bot owner. |  `@Shinobu do you love me`
`@BotName how are you`, `@BotName how are you?`  |  Replies positive only if bot owner is online. |  `@Shinobu how are you`
`@BotName fire`  |  Shows a unicode fire message. Optional parameter [x] tells her how many times to repeat the fire. |  `@Shinobu fire [x]`
`@BotName dump`  |  Dumps all of the invites it can to dump.txt.** Owner Only.** |  `@Shinobu dump`
`@BotName ab`  |  Try to get 'abalabahaha'| `@Shinobu ab`
...
`$$$`  |  Check how much ShinobuFlowers a person has. (Defaults to yourself) | `$$$` or `$$$ @Someone`
`$give`  |  Give someone a certain amount of ShinobuFlowers|`$give 1 "@SomeGuy"`
...
`$betroll`, `$br`  |  Bets a certain amount of ShinobuFlowers and rolls a dice. Rolling over 66 yields x2 flowers, over 90 - x3 and 100 x10. |  `$br 5`
...
`>gencurrency`, `>gc`  |  Toggles currency generation on this channel. Every posted message will have 2% chance to spawn a ShinobuFlower. Optional parameter cooldown time in minutes, 5 minutes by default. Requires Manage Messages permission. |  `>gc` or `>gc 60`
`>leet`  |  Converts a text to leetspeak with 6 (1-6) severity levels |  `>leet 3 Hello`
`>choose`  |  Chooses a thing from a list of things |  `>choose Get up;Sleep;Sleep more`
`>8ball`  |  Ask the 8ball a yes/no question. |  `>8ball should i do something`
`>rps`  |  Play a game of rocket paperclip scissors with Shinobu. |  `>rps scissors`
...
`>heal`  |  Heals someone. Revives those who fainted. Costs a ShinobuFlower |  `>heal @someone`
`>type`  |  Get the poketype of the target. |  `>type @someone`
`>settype`  |  Set your poketype. Costs a ShinobuFlower. |  `>settype fire`
